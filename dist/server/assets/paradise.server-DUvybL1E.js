import { x as getDefaultExportFromCjs } from "./server-D8pWyu0D.js";
...
const BASE_URL = process.env.PARADISE_BASE_URL || "https://multi.paradisepags.com";
const DEFAULT_PRODUCT_HASH = process.env.PARADISE_PRODUCT_HASH || "prod_40e485d44e117d3a";
function getApiKey() {
...
async function createPixCharge(input) {
  ...
  const transactionId = json?.hash ?? json?.external_id ?? json?.id ?? json?.transaction?.hash ?? json?.data?.hash ?? json?.data?.id ?? "";
  const copyPaste = json?.pix?.copy_paste ?? json?.pix?.copyPaste ?? json?.pix?.qrcode ?? json?.pix?.qrCode ?? json?.pix?.payload ?? json?.qr_code ?? json?.copy_paste ?? "";
  const qrCodeBase64Raw = json?.pix?.qr_code_base64 ?? json?.pix?.qrCodeBase64 ?? json?.qr_code_base64 ?? "";
  const qrCodeBase64 = String(qrCodeBase64Raw).replace(
    /^data:image\/png;base64,/,
    ""
  );
  const expiresAt2 = json?.pix?.expiration_date ?? json?.pix?.expirationDate ?? json?.pix?.expiresAt ?? null;
  return {
...
async function getTransactionStatus(transactionId) {
  ...
  const raw = json?.payment_status ?? json?.status ?? json?.data?.status ?? json?.data?.payment_status ?? "pending";
...
export {
  ...
  arrayType as c,
  createClient as d,
  createPixCharge as e,
  supabaseAdmin as f,
  getTransactionStatus as g,
  literalType as l,
  numberType as n,
  objectType as o,
  stringType as s
